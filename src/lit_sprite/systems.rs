...
use crate::lit_sprite::{components::LitSpriteDb, global_components::{LitSprite, LitSpriteMaterial}};
...
pub fn handle_new_lit_sprites(
    ...
    for (entity, sprite) in query.iter() {
        ...
        let material_key = (sprite.albedo_texture.as_ref().map(|h| h.id()), sprite.specular_texture.as_ref().map(|h| h.id()));
